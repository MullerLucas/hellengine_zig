...
const HelloTriangleApplication = struct {
    ...
    fn recreateSwapChain(self: *Self) !void {
        var size = glfw.Window.getFramebufferSize(self.window.?);

        while (size.width == 0 or size.height == 0) {
            size = glfw.Window.getFramebufferSize(self.window.?);
            glfw.waitEvents();
        }
    ...
    fn createSurface(self: *Self) !void {
        if ((glfw.createWindowSurface(self.instance, self.window.?, null, &self.surface)) != @enumToInt(vk.Result.success)) {
    ...
    fn getRequiredExtensions(allocator: Allocator) !std.ArrayListAligned([*:0]const u8, null) {
        ...
        try extensions.appendSlice(glfw.getRequiredInstanceExtensions() orelse @panic("failed to get extensions"));
