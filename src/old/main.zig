...
pub fn main() !void {
    ...
    const gc = try GraphicsContext.init(allocator, app_name, window.window);
    defer gc.deinit();
    ...
    while (!window.shouldClose()) {
        ...
        if (state == .suboptimal or extent.width != @intCast(u32, ext.width) or extent.height != @intCast(u32, ext.height)) {
